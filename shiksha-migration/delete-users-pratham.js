/**
 * delete-users-pratham.js
 *
 * Deletes inactive users from the Pratham User Microservice DB.
 * Tables (in FK-safe order): child tables first → Users last
 *
 * Usage:
 *   node delete-users-pratham.js --dry-run    (count only, no deletes)
 *   node delete-users-pratham.js --live       (actual deletion)
 *
 * Input:  shiksha-migration/users-to-delete.csv  (header: userId, one UUID per row)
 * Output: shiksha-migration/logs/deletion-pratham-YYYY-MM-DD.json
 */
...
const CHECKPOINT_PATH = path.join(LOGS_DIR, 'checkpoint-pratham.json');

// Deletion order respects FK constraints: children first, parent (Users) last.
// Column names verified against entity definitions in src/:
//   - FieldValues       -> linked by "itemId" (NOT "userId"); see fields-values.entity.ts
//   - CohortMembers     -> "userId"; cohort-member.entity.ts
//   - AutomaticMember   -> "userId"; automatic-member.entity.ts
//   - UserRolesMapping  -> "userId"; assign-role.entity.ts  (@Entity name has an 's')
//   - UserTenantMapping -> "userId"; user-tenant-mapping.entity.ts
//   - Users             -> "userId" (PK); user-entity.ts
const TABLE_ORDER = [
  { name: 'FieldValues',       column: 'itemId' },
  { name: 'UserRolesMapping',  column: 'userId' },
  { name: 'UserTenantMapping', column: 'userId' },
...
function parseFlag() {
  ...
  console.error('  node delete-users-pratham.js --dry-run');
  console.error('  node delete-users-pratham.js --live');
  process.exit(1);
}
...
function loadUserIds() {
  ...
  const lines = fs.readFileSync(CSV_PATH, 'utf8')
    ...
    .filter(l => l && l.toLowerCase() !== 'userid'); // Remove header if present
...
function writeLog(logObj) {
  ...
  const filePath = path.join(LOGS_DIR, `deletion-pratham-${todayStr()}.json`);
...
async function main() {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  DELETE-USERS-PRATHAM  [${mode.toUpperCase()}]`);
  console.log(`  DB: ${dbConfig.source.database} @ ${dbConfig.source.host}`);
  ...
  console.log('Connected to Pratham User DB\n');
  ...
  const logObj = {
    script: 'delete-users-pratham.js',
    mode,
    database: dbConfig.source.database,
    host: dbConfig.source.host,
    startedAt,
    finishedAt,
